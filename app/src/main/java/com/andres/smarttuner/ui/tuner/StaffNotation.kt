...
import com.andres.smarttuner.ui.components.TunerCard
...
fun StaffCard(state: TunerUiState, color: Color, modifier: Modifier = Modifier) {
    ...
    TunerCard(modifier) {
        Row(verticalAlignment = Alignment.CenterVertically) {
            SectionLabel(stringResource(R.string.staff_title))
            Spacer(Modifier.weight(1f))
            Text(clefName, color = colors.textMuted, style = TunerTheme.typography.footnote)
        }
        StaffNotation(
            position = position,
            accidental = note?.accidentalSymbol.orEmpty(),
            label = note?.label,
            color = if (state.hasSignal) color else colors.textMuted,
            modifier = Modifier
                .fillMaxWidth()
                .height(120.dp),
        )
...
fun StaffNotation(
    ...
    Canvas(modifier) {
        ...
        for (line in 0..4) {
            val y = yOf(line * 2f)
            drawLine(colors.staffLine, Offset(startX, y), Offset(endX, y), strokeWidth = 1.2.dp.toPx())
        }
        drawLine(colors.staffLine, Offset(startX, yOf(0f)), Offset(startX, yOf(8f)), strokeWidth = 1.2.dp.toPx())
        drawLine(colors.staffLine, Offset(endX, yOf(0f)), Offset(endX, yOf(8f)), strokeWidth = 2.5.dp.toPx())

        drawClef(textMeasurer, clef, halfSpace, colors.textPrimary.copy(alpha = 0.85f), ::yOf)
        ...
        // Alteración (su tamaño depende del pentagrama, no de la escala de texto).
        ...
        position.octaveShiftLabel?.let { shift ->
            val layout = textMeasurer.measure(shift, typography.staffMark)
            val shiftY = if (position.octaveShift > 0) yOf(12f) - layout.size.height else yOf(-4f)
            drawText(layout, color = colors.textMuted, topLeft = Offset(headX - layout.size.width / 2f, shiftY))
        }

        // Nombre de la nota junto a la figura.
        val nameLayout = textMeasurer.measure(label, typography.staffNote)
        drawText(
